 ...
 * @author Eric Dubuis
 */
public class FactoryForXStream implements Factory {
	...
	public Person makePerson(String name) {
		return new PersistablePerson(name);
	}
	...
	public Resource makeResource(String name) {
		return new PersistableResource(name);
	}
	...
	public Reservation makeReservation(Set<Resource> rs, Person p, DateRange dr) {
		Reservation res = new PersistableReservation(rs, p, dr);
		for (Resource r : rs) {
